...
const FetchData_Search=async(SearchValue)=>{
  try {
    let Resp=await fetch(`http://localhost:1012/Travel?search=${SearchValue}&limit=3`)
...
// <=================== Display Data -------------------->
    ...
    async function TotalPage() {
        try {
            let Resp = await axios.get(`http://localhost:1012/Travel`);
            console.log(Resp.data)
            setTotalPage(Resp.data.Country)
        } catch (error) {
            console.log(error)
        }
    }
    ...
    const FetchData = async () => {
        let Apiurl = getUrl(
            `http://localhost:1012/Travel?page=${page}&limit=${limit}`,
            sortby
        );
        try {
            let Resp = await axios.get(Apiurl);
            console.log(Resp.data)
            setArr(Resp.data.Country)
        } catch (error) {
            console.log(error)
        }
    }
    ...
    return (
      ...
      <div>
          ...
          <div className={style.filterbtn} style={{display:"flex",justifyContent:"space-evenly"}} >
      <ButtonGroup spacing="4"className={style.btnGrp}>
        {/* <Button onClick={()=>setAll("")} colorScheme={sortby === '' ? 'gray' : 'blue'}>
          All
        </Button>
        ...
        <Button onClick={() => setSortby('asc')} colorScheme={sortby === 'asc' ? 'gray' : 'blue'}>
          Price Low To High
        </Button>
        <Button onClick={() => setSortby('desc')} colorScheme={sortby === 'desc' ? 'gray' : 'blue'}  >
          Price High To Low
        </Button>
      </ButtonGroup>
    </div>
